    ...
    const cb = (declaration) => {
        ...
        for (const element of declaration.importClause.namedBindings.elements) {
            ...
            if (!ts.isIdentifier(element.name)) {
                return ctx.addFailureAtNode(element, element.getFullText(sf) + Rule.SYMBOL_NOT_FOUND_FAILURE_STR);
            }
            ...
            // are being looked at.
            const [, moduleName] = sourceFile.match(ANGULAR_MATERIAL_FILEPATH_REGEX) || [];
            if (!moduleName) {
                return ctx.addFailureAtNode(element, element.getFullText(sf) + Rule.SYMBOL_FILE_NOT_FOUND_FAILURE_STR);
            }
        ...
        // recommended fix.
        ctx.addFailureAtNode(declaration.moduleSpecifier, Rule.ONLY_SUBPACKAGE_FAILURE_STR, new Lint.Replacement(declaration.getStart(sf), declaration.getWidth(), fix));
    };
 ...
 */
function buildSecondaryImportStatements(importMap) {
