...
(function (factory) {
    ...
    class HammerGesturesRule extends schematics_2.MigrationRule {
        ...
        _setupNewGestureConfigInRootModule(project, gestureConfigPath) {
            ...
            const metadata = schematics_2.getDecoratorMetadata(sourceFile, 'NgModule', '@angular/core');
            // If no "NgModule" definition is found inside the source file, we just do nothing.
            if (!metadata.length) {
                return;
            }
            const recorder = this.getUpdateRecorder(sourceFile.fileName);
            const providersField = schematics_2.getMetadataField(metadata[0], 'providers')[0];
            ...
            if (!providerIdentifiers ||
                ...
                schematics_2.addSymbolToNgModuleMetadata(sourceFile, relativePath, 'providers', this._printNode(newProviderNode, sourceFile), null)
                    .forEach(change => {
        ...
        _setupHammerModuleInRootModule(project) {
            ...
            const importsField = schematics_2.getMetadataField(metadata[0], 'imports')[0];
            ...
            if (!importIdentifiers ||
                ...
                schematics_2.addSymbolToNgModuleMetadata(sourceFile, relativePath, 'imports', this._printNode(hammerModuleExpr, sourceFile), null)
                    .forEach(change => {
                    if (change instanceof change_1.InsertChange) {
                        recorder.insertRight(change.pos, change.toAdd);
                    }
