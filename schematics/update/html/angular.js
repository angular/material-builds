...
function findInputsOnElementWithTag(html, inputName, tagNames) {
    ...
    return elements_1.findAttributeOnElementWithTag(html, `[${inputName}]`, tagNames).map(offset => offset + 1);
}
...
function findInputsOnElementWithAttr(html, inputName, attrs) {
    ...
    // is part of the attribute and therefore also part of the offset. We only want to return
    // the offset for the inner name of the input.
    return elements_1.findAttributeOnElementWithAttrs(html, `[${inputName}]`, attrs).map(offset => offset + 1);
}
