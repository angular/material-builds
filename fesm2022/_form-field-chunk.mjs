...
import { Directive, InjectionToken, inject, Input, ElementRef, NgZone, Renderer2, ViewChild, ViewEncapsulation, Component, ChangeDetectorRef, isSignal, viewChild, computed, contentChild, signal, effect, afterRenderEffect, ContentChildren, ContentChild } from '@angular/core';
...
class MatFormField {
  ...
  ngAfterContentChecked() {
    ...
    if (this._control !== this._previousControl) {
      ...
      if (this._control.ngControl && this._control.ngControl.control) {
        this._previousControlValidatorFn = isFieldTree(this._control.ngField) ? null : this._control.ngControl.control.validator;
      }
      this._previousControl = this._control;
      this._changeDetectorRef.markForCheck();
    }
    if (this._control.ngControl && this._control.ngControl.control && !isFieldTree(this._control.ngField)) {
  ...
  _initializeControl(previousControl) {
    ...
    this._describedByChanges?.unsubscribe();
    this._describedByChanges = control.stateChanges?.pipe(startWith([undefined, undefined]), map(() => [this._unwrapMaybeSignal(control.errorState), control.userAriaDescribedBy]), pairwise(), filter(([[prevErrorState, prevDescribedBy], [currentErrorState, currentDescribedBy]]) => {
  ...
  _getSubscriptMessageType() {
    return this._errorChildren && this._errorChildren.length > 0 && this._unwrapMaybeSignal(this._control.errorState) ? 'error' : 'hint';
  }
  _handleLabelResized() {
